...
export function TeamGoalAdminClient() {
    ...
    if (isLoading) return <div className="h-64 animate-pulse bg-white rounded-xl border border-ash-100" />

    const activeGoals = goals.filter(g => g.status === 'active')
    const completedGoals = goals.filter(g => g.status === 'completed')

    return (
        <div className="bg-white rounded-xl border border-ash-200 overflow-hidden shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-ash-200">
                {/* Create Form */}
                <div className="p-6 col-span-1 bg-ash-50/50">
                    <div className="flex items-center gap-2 mb-6">
                        <Plus className="h-5 w-5 text-brand-orange" />
                        <h2 className="text-lg font-bold text-ash-900">Nuovo Obiettivo</h2>
                    </div>

                    <form onSubmit={handleCreate} className="space-y-4">
                        <div>
                            <label className="block text-xs font-semibold text-ash-600 mb-1">Titolo (es. Target Febbraio)</label>
                            <input
                                required type="text" value={title} onChange={e => setTitle(e.target.value)}
                                className="w-full text-sm border-ash-300 rounded-lg focus:ring-brand-orange focus:border-brand-orange px-3 py-2 border shadow-sm"
                                placeholder="Titolo dell'obiettivo"
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-ash-600 mb-1">Target Appuntamenti</label>
                            <input
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-ash-600 mb-1">Premio Comunitario (a tutti)</label>
                            <div className="relative">
                                <Coins className="h-4 w-4 absolute left-3 top-2.5 text-gold-500" />
                                <input
                                    required type="number" min="1" value={rewardCoins} onChange={e => setRewardCoins(Number(e.target.value))}
                                    className="w-full text-sm border-ash-300 rounded-lg focus:ring-brand-orange focus:border-brand-orange pl-9 pr-3 py-2 border shadow-sm"
                                    placeholder="Fenice Coin per GDO"
                                />
                            </div>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-ash-600 mb-1">Tipo di Obiettivo</label>
                            <select
                                value={goalType} onChange={e => setGoalType(e.target.value as any)}
                                className="w-full text-sm border-ash-300 rounded-lg focus:ring-brand-orange focus:border-brand-orange px-3 py-2 border shadow-sm bg-white"
                            >
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-ash-600 mb-1">Scadenza</label>
                            <input
                ...
                <div className="p-6 col-span-2 space-y-8">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 border-b border-ash-100 pb-2">
                            <Target className="h-5 w-5 text-brand-orange" />
                            <h3 className="text-sm font-semibold text-ash-400 uppercase tracking-widest">Obiettivi Attivi ({activeGoals.length})</h3>
                        </div>
                        {activeGoals.length === 0 ? (
                            <p className="text-sm text-ash-500 py-4">Nessun obiettivo attivo al momento.</p>
                        ) : (
                                ...
                                {activeGoals.map(goal => (
                                    <div key={goal.id} className="border border-brand-orange/30 bg-orange-50/30 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-start justify-between relative group">
                                        ...
                                        <div className="pr-8 md:pr-12">
                                            ...
                                            <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-ash-600">
                                                <span className="flex items-center gap-1 bg-white border border-ash-200 px-2 py-1 rounded-md shadow-sm">
                                                ...
                                                <span className="flex items-center gap-1 bg-gold-100 text-gold-800 border border-gold-200 px-2 py-1 rounded-md shadow-sm">
                                                    <Coins className="h-3.5 w-3.5 text-gold-600" /> Premio: {goal.rewardCoins} Coin a testa
                                                </span>
                                                <span className="flex items-center gap-1 bg-white border border-ash-200 px-2 py-1 rounded-md shadow-sm">
                                                    <Calendar className="h-3.5 w-3.5 text-orange-500" /> Scade il: {new Date(goal.deadline).toLocaleDateString()}
                                                </span>
                                            </div>
                                        </div>
                                        {/* Progress Bar local to the card */}
                                        <div className="min-w-[150px] w-full md:w-auto mt-2 md:mt-0">
                                            <div className="h-2 w-full bg-ash-200 rounded-full overflow-hidden mb-1">
                                                <div
                    ...
                    {completedGoals.length > 0 && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 border-b border-ash-100 pb-2">
                                ...
                                <h3 className="text-sm font-semibold text-ash-400 uppercase tracking-widest">Obiettivi Completati</h3>
                            </div>
                                ...
                                {completedGoals.map(goal => (
                                    <div key={goal.id} className="border border-green-200 bg-green-50 rounded-xl p-3 flex items-center justify-between opacity-75">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-ash-900 text-sm">{goal.title}</h4>
