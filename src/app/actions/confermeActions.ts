...
export async function assignWebinarLeadToSalesperson(leadId: string, salespersonId: string, newAppointmentDate?: Date) {
    ...
    const [sp] = await db
        ...
        .from(users)
        .where(and(eq(users.id, salespersonId), eq(users.role, 'VENDITORE')))
        .limit(1);
    ...
    await createGoogleCalendarEvent(
        ...
        leadId,
        "appointment"
    ).catch((err: any) => console.error("Could not create calendar event:", err?.message || err));
    ...
    // Gamification — questa conta come una conferma per l'utente Conferme.
