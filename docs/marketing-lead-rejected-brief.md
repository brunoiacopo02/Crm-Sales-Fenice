...
| `automatic` | boolean | `true` **solo** per lo scarto automatico dopo il terzo tentativo a vuoto. Vedi §5. |
...
| `callCount` | integer | Quante chiamate ha ricevuto il lead prima di essere scartato. |
...
| `rejectedBy` | oggetto \| null | Operatore che ha scartato. `null` per gli scarti automatici di sistema. |
...
Quando un lead non risponde per tre chiamate consecutive, il CRM lo scarta da solo con
`reasonCode: "UNREACHABLE"`, `automatic: true` e `rejectedBy: null`.
...
Possiamo inviare eventi di prova sull'endpoint di test
(`.../crm-fenice-test`, secret `WEBHOOK_SECRET_CRM_TEST`) prima di accendere la
produzione. Ditecelo e li mandiamo.
