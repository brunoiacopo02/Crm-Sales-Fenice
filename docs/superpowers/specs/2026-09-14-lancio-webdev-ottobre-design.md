...
- **Riscaldamento del secondo numero ("Account fenice 2", account Twilio e Business Manager separati)**: i benvenuti partono dal secondo numero con un **tetto di 50 al giorno** (`LANCIO_WELCOME_MAX_PER_DAY` sul mittente secondario); le agende sul secondo numero si decidono in un'altra sessione. Il resto dei benvenuti (oltre i 50) resta differito finché il PO non alza il tetto o non riapre il numero principale (dal 21/09).
- **Link Zoom del 5/10: solo a chi ha risposto almeno una volta** (`posto_bloccato` o `attesa` con inbound). Chi non ha mai risposto lo riceve **solo se lo chiede** (il bot risponde al suo inbound con il link, fase assistenza). Niente blast ai freddi = niente rischio ban.
